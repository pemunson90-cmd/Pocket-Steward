...
**What's verified vs. reviewed-only, and why the difference matters:** `FileCategory.kt`, `FileRefCodec.kt` (the checkpoint queue's serialize/deserialize), and the pure model types in `StorageModels.kt`/`StorageGateway.kt` have zero Android imports, so I pulled them into a throwaway plain-Kotlin Gradle module (`kotlin("jvm")`, JUnit + Truth from Maven Central, no `google()` needed) and actually compiled and ran the tests against a real Kotlin 2.3.20 compiler in this sandbox — all 5 pass, including a round-trip test with a content URI containing colons, which is exactly the kind of edge case I wouldn't trust myself to get right blind. That process caught nothing wrong in those files, but doing it on `SafStorageGateway` surfaced a real bug before it shipped: `DocumentFile.fromTreeUri` only ever resolves a tree's *root* document no matter what URI you pass it, so the original `fromTreeUri(...) ?: fromSingleUri(...)` fallback would have silently re-listed the root instead of the requested child for every directory past the first level. Fixed by normalizing every `FileRef.Saf` — including the root — to document-URI form (`DocumentsContract.buildDocumentUriUsingTree`) so `fromSingleUri` alone handles every node uniformly. `FileScanner`, `AppDatabase`, both gateways, and the new UI still can't be compiled in this sandbox (Room and Compose aren't on Maven Central) — reviewed carefully by hand, not proven.

**Exit criterion not yet confirmed on-device:** scanning Downloads/Documents/Pictures and seeing accurate counts/sizes. That needs the same real-build-and-sideload round trip Milestone 0 went through.
