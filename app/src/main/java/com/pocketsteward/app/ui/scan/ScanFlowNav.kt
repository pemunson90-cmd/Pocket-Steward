...
object ScanFlow {
    ...
    const val ARG_RESTORE_LAST = "restoreLast"

    /** Entry route, optionally carrying a Home action, bounded request, or saved workflow id. */
    const val ENTRY =
        "scan_flow/scan?$ARG_ACTION={$ARG_ACTION}&$ARG_REQUEST={$ARG_REQUEST}&$ARG_WORKFLOW={$ARG_WORKFLOW}&$ARG_SAVED_SEARCH={$ARG_SAVED_SEARCH}&$ARG_IMPORTED_PLAN={$ARG_IMPORTED_PLAN}&$ARG_SCHEDULED_REVIEW={$ARG_SCHEDULED_REVIEW}&$ARG_RESTORE_LAST={$ARG_RESTORE_LAST}"
    ...
    fun entryWithLastScan(): String = "scan_flow/scan?$ARG_RESTORE_LAST=1"
}
    ...
    navigation(startDestination = ScanFlow.ENTRY, route = ScanFlow.GRAPH) {
        composable(
            ...
            arguments = listOf(
                ...
                navArgument(ScanFlow.ARG_RESTORE_LAST) {
                    type = NavType.StringType
                    nullable = true
                    defaultValue = null
                },
            ...
            ScanScreen(
                ...
                autoRestoreLast = entry.arguments?.getString(ScanFlow.ARG_RESTORE_LAST) == "1",
